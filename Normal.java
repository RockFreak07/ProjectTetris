...
public class Normal extends JPanel implements KeyListener, MouseListener, MouseMotionListener{
	...
	private int[][] normal = new int[boardHeight][boardWidth];
	...
	public Normal(){
		// load Assets
		blocks = ImageLoader.loadImage("/tiles1.png");
		
		background = ImageLoader.loadImage("/bg4.jpg");
		pause = ImageLoader.loadImage("/pause1.png");
		refresh = ImageLoader.loadImage("/refresh1.png");
	...
	public void paintComponent(Graphics g){
		...
		{
			for(int col = 0; col < normal[row].length; col ++)
			{
				
				if(normal[row][col] != 0)
				{
					
					g.drawImage(blocks.getSubimage((normal[row][col] - 1)*blockSize,
		...
		//time
		//g.drawString("Time", Window.WIDTH - 125, Window.WIDTH - 100);
		//g.drawString(time+"", Window.WIDTH - 125, Window.HEIGHT/2 + 30);

		g.drawString("SCORE", Window.WIDTH - 125, Window.WIDTH - 125);
	...
	public void setCurrentShape(){
		...
		for(int row = 0; row < currentShape.getCoords().length; row ++)
		{
			for(int col = 0; col < currentShape.getCoords()[0].length; col ++)
			{
				...
				{
					if(normal[currentShape.getY() + row][currentShape.getX() + col] != 0)
						gameOver = true;
				}
			}		
		}
